...
final class PresentationTest: XCTestCase {
  ...
  func testSDJWTPresentationWithSelectiveDisclosuresUsingClaimPaths() async throws {
    ...
    let issuerSignedSDJWT = try await SDJWTIssuer.issue(
      ...
      FlatDisclosedClaim("my_count_array", ["one", "two", "three"])
      ArrayClaim("my_array") {
        SdElement.flat("one")
        SdElement.flat("two")
        SdElement.flat("three")
      }
    }
    ...
    let query: Set<ClaimPath> = Set([
      ...
      .claim("evidence").arrayElement(1).claim("the_evidence").arrayElement(0).claim("time"),
      .claim("my_count_array")
      ]
    )
    ...
    XCTAssertNotNil(kbJwt)
    XCTAssertEqual(presentedSdJwt.disclosures.count, 8)
